...
本篇把默认的磁盘后端换成实现 `SandboxBackendProtocol` 的阿里云沙箱，执行落在隔离的云端运行时中。按 `thread_id` 复用沙箱实例；用键值存储（LangGraph 的 `BaseStore`）记下该线程当前的 `sandbox_id`。结构化输出采用 `ToolStrategy`：把结构化 schema 挂成一个「提交结果」的工具，模型结束时调用它，框架校验后写入最终响应。图入口即 Agent Server 登记的 `coding_agent` 图。
...
| 概念                     | 作用                                                                                                             |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| `SandboxBackendProtocol` | 在文件后端协议 `BackendProtocol`（文件工具依赖的存储协议）上增加 `id` 与 `execute` / `aexecute`。传入该类后端后，harness 才向模型暴露 `execute` |
| `AliyunSandboxBackend`   | 本篇实现。按 `thread_id` 调用沙箱工厂 `get_sandbox`，把文件与命令操作转到该沙箱                                  |
| `deploy`                 | 自定义工具。把工作区产物复制到 `/home/user/output`，返回入口 URL，并销毁沙箱                                     |
| `CodingAgentResponse`    | 最终结构化响应：`status`、`summary`、`artifacts`                                                                 |
| `ToolStrategy`           | 结构化输出策略。挂一个「提交结构化结果」的工具，模型按 schema 提交后写入最终响应                                 |

## 整体流程

用户提出编码需求后，Agent 在 Deep Agents 默认循环中调用内置文件工具与 `execute`。这些调用不直接接触宿主磁盘，而是经 `AliyunSandboxBackend` 转到按 `thread_id` 取得的阿里云沙箱。工作区 `/home/user/workspace` 与输出目录 `/home/user/output` 在创建时已挂载 OSS：沙箱内读写等价于桶内 `/{thread_id}/workspace` 与 `/{thread_id}/output`。开发完成后 Agent 调用 `deploy`，把产物复制到 output、校验入口文件、返回访问 URL，再 `sandbox.kill()`。最后用 `CodingAgentResponse` 向用户交付。
...
文件系统工具名不变，更换 `backend` 只更换读写落点：默认的 `StateBackend`（写入图状态）与 `FilesystemBackend`（写入本地磁盘）只提供文件工具；实现 `SandboxBackendProtocol` 后，同一套文件工具之外会多出 `execute`，命令在隔离运行时中执行。

## 实现阿里云沙箱后端

`langchain_project/backends/aliyun_sandbox_backend.py`：实现 `SandboxBackendProtocol`，把 Deep Agents 的文件与命令调用转到沙箱工厂 `get_sandbox`。`get_sandbox` 按 `key` 复用沙箱实例：Store 中存有该 `key` 的 `sandbox_id` 时连接复用，否则新建并回写；OSS 前缀按 `key` 固定为 `/{key}/workspace` 与 `/{key}/output`，实例销毁后同一 `key` 的文件仍在。
...
隔离单位以 `key` 为准：`id` 取 `configurable.thread_id`，`_get_sandbox` 以该值作为 `key` 调用 `get_sandbox`，并用 `get_runtime().store` 复用尚未销毁的实例。缺少 `thread_id` 或 runtime store 时直接报错，因此应经 Agent Server / Studio 调用，而不是在未注入配置的情况下直接 `invoke`。

`_sandbox_operation` 在每次操作前后调用 `set_timeout(TIMEOUT)`，把销毁倒计时从当前时刻重新起算，即在长步骤前刷新沙箱存活时限，避免执行中途被回收。`aexecute` 的 `timeout` 是单条命令的执行时限，默认 60 秒，与沙箱存活时限不是同一参数；工作目录固定为 `/home/user/workspace`。
...
`aread` 先按字节读取：能按 UTF-8 解码则经 `slice_read_response` 分页返回，与文件工具 `read_file` 的按行分页一致；否则以 base64 返回。`agrep` 按字面文本匹配，不是正则，与文件工具 `grep` 一致。`aedit` 先读全文，再用 `perform_string_replacement` 做精确替换后写回。
...
平台只提供静态资源部署。`path` 指向工作区中的产物（文件或目录），`target_dirname` 为对象存储上的目标目录名，`entry_files` 为该目录内的相对入口路径。output 目录在创建沙箱时已挂载 OSS，复制完成即写入 `/{thread_id}/output/<target_dirname>/`。公开地址按同一前缀拼接：`{PUBLIC_BASE_URL}/{thread_id}/output/{target_dirname}/{entry_file}`。
...
`langchain_project/agents/coding_agent.py`：用 `create_deep_agent` 组装 Deep Agent 入口，把 `backend` 换成 `AliyunSandboxBackend()`，并加上系统提示词与结构化输出。`tools` 为工程统一维护的工具列表，须包含上一节定义的 `deploy`。

`CodingAgentResponse` 约束最终交付形状：`completed` 必须带至少一个已部署产物；`cannot_deliver` 的 `artifacts` 必须为空。`ToolStrategy` 把该 schema 挂成一个「提交结果」工具，模型结束时调用它，框架校验后写入 `structured_response`。
