...
LangChain 用 `create_agent` 一次调用生成已编译的图。必填项是模型标识（`Provider:Model Name`）；工具、系统提示词、状态结构、context 结构、检查点与 Store 均为选填。返回对象可直接 `invoke`，也可登记为 Agent Server 的图入口。`checkpointer` 对应 LangGraph第07章的线程和检查点，`store` 对应 LangGraph第26章的 Store；经 Agent Server 部署时二者由服务注入，创建时无须传入。`context_schema` 对应 LangGraph第12章 Runtime 的运行时配置通道。
