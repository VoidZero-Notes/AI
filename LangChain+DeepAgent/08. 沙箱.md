![image-20260918000028696](https://ipic-typora.oss-cn-shenzhen.aliyuncs.com/jishu-img/2026-09-17-160029.png)
...
隔离强度取决于执行落点：直接在宿主机上运行命令，隔离取决于操作系统配置；放进独立的进程、容器或虚拟机，则由运行时边界保证隔离。本篇给出四类隔离方案，并以 E2B 作为托管沙箱的落地方式。
...
`langchain_project/core/config.py`：定义沙箱配置 `SandboxSettings`，用 pydantic-settings 从 `.env` 读取兼容 E2B 的接入参数与 OSS 挂载信息。
...
![image-20260918000052056](https://ipic-typora.oss-cn-shenzhen.aliyuncs.com/jishu-img/2026-09-17-160052.png)
...
![image-20260918000110597](https://ipic-typora.oss-cn-shenzhen.aliyuncs.com/jishu-img/2026-09-17-160111.png)
...
![image-20260918000131091](https://ipic-typora.oss-cn-shenzhen.aliyuncs.com/jishu-img/2026-09-17-160131.png)

`langchain_project/core/sandbox.py`：按 `key` 取得沙箱。两件事分开做：用键值存储（LangGraph 的 `BaseStore`）记下该 `key` 当前的 `sandbox_id`，以便复用尚未销毁的实例；把 OSS 的 `bucketPath` 写成 `/{key}/workspace` 与 `/{key}/output`，使文件按 `key` 落盘。未传入 `store` 时使用内存实现 `InMemoryStore`。
