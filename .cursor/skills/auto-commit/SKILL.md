...
description: >-
  ...
  笔记中出现密码、密钥、环境变量等内容仍可提交，但 commit message 完全不提这类内容。
---
...
- `scope` 可选；笔记类可用目录或主题简写（如 `agent`、`langgraph`、`workflow`）
- 有正文时，标题与正文之间空一行
- **敏感信息从 message 里直接忽略**：笔记里可以有密码、密钥、Token、环境变量、连接串等，文件照常提交；起草 message 时当作这些改动不存在，标题和正文都不要提具体值，也不要用「补充环境变量说明」这类概括去点明它们。只写其余可见改动；若整次改动只涉及这类内容，改用主题级概括（如「补充笔记」），仍不点明敏感项
...
```text
docs(agent): 补充工具调用与消息循环的说明
```

```text
docs(langgraph): 统一状态图章节体例并收敛空行
...
- **笔记中的敏感内容可以提交**：密码、密钥、Token、环境变量、连接串等出现在 `.md` 笔记里时，不要拦截提交；commit message（含向用户回报的那条）直接忽略这类内容，既不写具体值，也不提「环境变量 / 密钥 / 密码」等字样
